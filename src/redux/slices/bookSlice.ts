...
export const borrowBook = createAsyncThunk('book/borrow', async (object: Book) => {
  const res = await fetch('http://localhost:3000/Books/' + object.id, {
    method: 'PUT',
    body: JSON.stringify({
      ISBN: object.ISBN,
      title: object.title,
...
export const filterBookByName = createAsyncThunk('books/filterName', async (name: string) => {
  const res = await fetch(`http://localhost:3000/Books?title_like=${name}`, {
    method: 'GET',
    headers: {
      'Content-type': 'application/json'
    }
...
export const filterBookByAuthor = createAsyncThunk('books/filterAuthor', async (name: string) => {
  const res = await fetch(`http://localhost:3000/Books?author_like=${name}`, {
    method: 'GET',
    headers: {
      'Content-type': 'application/json'
    }
